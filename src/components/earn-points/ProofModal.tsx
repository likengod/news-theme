...
export function ProofModal({ task, onSubmit, onClose }: Props) {
  ...
  const submit = () => {
    if (!visited) {
      toast.error(
        `Please visit and ${task.actionLabel.toLowerCase()} our ${task.platform} page first`,
      );
      return;
    }
        ...
        <div className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-slate-800">
                  Visit {task.platform} & {task.actionLabel}
                </p>
                <p className="text-xs text-slate-500">
                  Open the page and actually {task.actionLabel.toLowerCase()} us
                </p>
              </div>
              <a
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-sm font-semibold text-slate-800">
                Enter your {task.platform} handle
              </p>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <span className="text-xs text-slate-700">
                  I confirm I have <strong>{task.actionLabel.toLowerCase()}ed</strong> the News
                  Theme {task.platform} page using the account{" "}
                  <strong>{handle || "provided above"}</strong>. I understand that false claims will
                  result in points being revoked and account suspension.
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 rounded-lg bg-blue-50 px-3 py-2.5 text-xs text-blue-700">
            ...
            <span>
              Points are credited after admin verification (24–48 hrs). Fake submissions will be
              permanently banned from the rewards program.
            </span>
          </div>
