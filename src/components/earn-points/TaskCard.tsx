...
export function TaskCard({
  ...
  return (
    <div
      className={`flex items-center justify-between gap-3 rounded-xl border p-4 transition-all ${cardCls}`}
    >
        ...
        <div className="min-w-0">
          ...
          {isPending && (
            <p className="text-[11px] text-amber-600 font-medium mt-0.5">
              ⏳ Under review — submitted as <em>{pending.handle}</em>
            </p>
          )}
          {isRejected && (
            <p className="text-[11px] text-red-600 font-medium mt-0.5">
              ❌ Rejected — please resubmit with correct handle
            </p>
          )}
      ...
      {done || isApproved ? (
        <span className="shrink-0 rounded-full bg-emerald-100 px-3 py-1 text-[11px] font-bold uppercase text-emerald-700">
          Claimed
        </span>
      ) : isPending ? (
