...
export async function createTablesAndIndexes(
  ...
  // Schema upgrade: Add show_in_header and sort_order if missing
  try {
  ...
  try {
    ...
    if (!err.message.includes("Duplicate column name")) {
      console.error("Migration error adding sort_order:", err.message);
    }
  }
  ...
  // Ensure extended journalist columns exist on profiles table (retroactive update)
  const extCols = await query("SHOW COLUMNS FROM profiles LIKE 'father_name'");
  if (extCols.length === 0) {
    const colDefs = [
    ...
    for (const def of colDefs) {
      try {
        ...
        if (!e.message.includes("Duplicate column name")) {
          console.error("Migration error adding column to profiles:", e.message);
        }
      }
    }
  }
  ...
  // 9.7 Create media_library table
  await query(`
  ...
  await safeCreateIndex(
    "articles",
    "idx_articles_hero_flags",
    "featured, status, date DESC",
  );
  await safeCreateIndex(
    "comments",
    "idx_comments_article_status",
    "article_slug, status, created_at DESC",
  );
  ...
  // Optimized Composite Indexes for High-Traffic Queries
