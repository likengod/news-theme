...
export const getPendingClaimsServer = createServerFn({ method: "GET" }).handler(
  async (): Promise<PendingClaim[]> => {
    try {
      const rows = await query(
        "SELECT value FROM site_settings WHERE setting_key = 'social_claims_data'",
      );
...
export const updateClaimStatusServer = createServerFn({ method: "POST" })
  ...
  .validator(
    (data: { userId: string; claimId: string; status: "approved" | "rejected"; points?: number }) =>
      data,
  )
  .handler(async ({ data }) => {
    ...
    const claim = claims.find(
      (c) => c.id === claimId || (c.userId === userId && c.platform === claimId),
    );
    if (claim) {
      claim.status = status;
    }
    ...
    if (status === "approved" && points > 0 && userId) {
      await query("UPDATE profiles SET points = COALESCE(points, 0) + ? WHERE id = ?", [
        points,
        userId,
      ]);
    }
