...
export const listAdminUsers = createServerFn({ method: "GET" })
  ...
  .validator(
    (data?: { q?: string; role?: string; sort?: string; page?: number; limit?: number }) =>
      data ?? {},
  )
  .handler(
    async ({
...
export const bulkToggleAdminUserBan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const placeholders = validIds.map(() => "?").join(",");
    await query(`UPDATE profiles SET active = ? WHERE id IN (${placeholders})`, [
      data.suspend ? 0 : 1,
      ...validIds,
    ]);
...
export const regeneratePublicUserId = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      try {
        await query("UPDATE profiles SET public_user_id = ? WHERE id = ?", [
          candidate,
          data.userId,
        ]);
...
export const setUserPoints = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((data: { userId: string; mode: "set" | "add"; amount: number }) => {
...
export const updateAdminUserDetails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.email) {
      ...
      const check = await query("SELECT id FROM users WHERE email = ? AND id != ?", [
        data.email,
        data.userId,
      ]);
