import { t as getServerFnById } from "./__23tanstack-start-server-fn-resolver-CYUfKARV.js";
...
function injectReelAds(items, ads, interval = 3) {
...
function readRaw(slot) {
	...
	try {
		...
		const parsed = raw ? JSON.parse(raw) : DEFAULTS[slot];
	...
	try {
		...
		if (totalChars > 2e6) {
			...
			if (rawMedia) try {
				...
				if (Array.isArray(media) && media.length > 2) localStorage.setItem("nt_media_library_v1", JSON.stringify(media.slice(0, 2)));
			} catch {
				localStorage.removeItem("nt_media_library_v1");
			}
			localStorage.removeItem("nt:site-ads-trash");
