...
var getAdminComments = createServerFn({ method: "GET" }).middleware([requireAdmin]).validator((data) => data ?? {}).handler(getAdminComments_createServerFn_handler, async ({ data }) => {
	...
	const [countRes, rows] = await Promise.all([query(`SELECT COUNT(*) AS total FROM comments${filterSql}`, params), query(`SELECT id, article_slug, article_title, user_name, user_email, body, status, created_at, parent_id
