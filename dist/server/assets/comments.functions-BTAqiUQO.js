...
import { u as getSiteSettingsServer } from "./site-content-CF9JiUW9.js";
...
var generateDummyCommentsFn = createServerFn({ method: "POST" }).middleware([requireAuth]).validator((d) => d).handler(generateDummyCommentsFn_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		const modelsToTry = [
			...
			{ url: `https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key=${settings.geminiApiKey}` }
		];
		...
		for (const model of modelsToTry) {
			...
			if (res.ok) break;
			lastError = resData.error?.message || res.statusText;
			resData = null;
		}
		if (!resData) throw new Error(lastError);
...
//#endregion
export { deleteComment_createServerFn_handler, generateDummyCommentsFn_createServerFn_handler, getAdminComments_createServerFn_handler, getArticleComments_createServerFn_handler, postArticleComment_createServerFn_handler, updateCommentStatus_createServerFn_handler };

//# sourceMappingURL=comments.functions-BTAqiUQO.js.map