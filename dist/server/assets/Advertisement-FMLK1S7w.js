...
import { g as loadAds, h as loadAdSlotScript, m as loadAdSlotMode, p as loadAdRotation } from "./site-content-B6BH1ZP8.js";
import { a as useSiteSettings, n as useAdSettings } from "./AdSettingsContext-BWPkkts7.js";
import { n as currentRoleSeesPopups } from "./roles-DZCyISYS.js";
...
function Advertisement({ slot, href = "#", label = "Sponsored", image, video, poster, title, caption, slides, intervalMs = 4e3, aspectRatio = "3 / 4" }) {
	...
	const items = slot ? slotMode === "script" ? [{
		...
		let img = s.image;
		if (slot === "home1" || slot === "ad3" || slot === "popup" || slot === "reel_ads") img = s.imagePortrait || (s.orientation === "portrait" ? s.image : "") || (s.imageLandscape && s.image === s.imageLandscape ? "" : s.image);
		else if (slot === "home2" || slot === "leaderboard") img = s.imageLandscape || (s.orientation === "landscape" ? s.image : "") || (s.imagePortrait && s.image === s.imagePortrait ? "" : s.image);
		return {
			image: img,
			href: s.href
		};
	}).filter((s) => !!s.image) : slides && slides.length > 0 ? slides : image || video ? [{
		image,
		video,
		poster,
		href
...
//# sourceMappingURL=Advertisement-FMLK1S7w.js.map