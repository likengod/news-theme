...
function isEnterpriseLicense(s) {
	if (!isLicenseActive(s)) return false;
	return (s?.licenseType || "").toLowerCase().trim().includes("enterprise");
}
function isEnterprisePlusLicense(s) {
	if (!isLicenseActive(s)) return false;
	const plan = (s?.licenseType || "").toLowerCase().trim();
	return plan === "enterprise plus" || plan === "enterprise+" || plan === "enterprise-plus" || plan.includes("enterprise plus") || plan.includes("enterprise+");
}
var defaultSettings = {
