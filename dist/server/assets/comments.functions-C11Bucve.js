...
import { u as getSiteSettingsServer } from "./site-content-huzqzZ3V.js";
...
var generateDummyCommentsFn = createServerFn({ method: "POST" }).middleware([requireAuth]).validator((d) => d).handler(generateDummyCommentsFn_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (!res.ok && resData.error?.message?.includes("is not found")) {
			res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${settings.geminiApiKey}`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(payload)
			});
...
//# sourceMappingURL=comments.functions-C11Bucve.js.map