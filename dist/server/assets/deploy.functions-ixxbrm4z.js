...
var gitPull = createServerFn({ method: "POST" }).handler(gitPull_createServerFn_handler, async () => {
	...
	setTimeout(async () => {
		try {
			...
			if (process.platform !== "win32") spawn("sh", ["-c", `sleep 2 && if ! fuser 3000/tcp >/dev/null 2>&1; then PORT=3000 APP_PORT=3000 nohup ${process.argv[0]} server.js > server.log 2>&1 & fi`], {
				detached: true,
				stdio: "ignore",
				cwd: ROOT
			}).unref();
			else spawn("cmd.exe", ["/c", `timeout /t 2 /nobreak >nul & set PORT=3000 & "${process.argv[0]}" server.js`], {
				detached: true,
				stdio: "ignore",
				cwd: ROOT
			}).unref();
...
//# sourceMappingURL=deploy.functions-ixxbrm4z.js.map