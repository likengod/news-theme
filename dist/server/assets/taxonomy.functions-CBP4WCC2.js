...
var getCategories = createServerFn({ method: "GET" }).validator((data) => data ?? {}).handler(getCategories_createServerFn_handler, async ({ data }) => {
	try {
		...
		if (!q && cachedCategories && cachedCategories.expiry > Date.now()) return cachedCategories.data;
		let sql = `
      ...
      `;
		...
		if (!q) cachedCategories = {
			data: mapped,
			expiry: Date.now() + 60 * 1e3
		};
		return mapped;
	} catch (err) {
...
var saveCategory = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((data) => data).handler(saveCategory_createServerFn_handler, async ({ data }) => {
	clearTagsCache();
	...
	if (c.id && c.id < 1e6) {
		...
		const res = await query(`INSERT INTO categories (name, slug, description, meta_title, meta_description, show_in_header, sort_order) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
			c.name,
			slug,
			c.description || "",
...
var deleteCategory = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((id) => id).handler(deleteCategory_createServerFn_handler, async ({ data: id }) => {
	clearTagsCache();
	clearCategoriesCache();
...
var importCategories = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((cats) => cats).handler(importCategories_createServerFn_handler, async ({ data: cats }) => {
	clearTagsCache();
	clearCategoriesCache();
	...
	for (const c of cats) {
		...
		if (existing.length > 0) {
			...
			await query(`UPDATE categories SET name = ?, slug = ?, description = ?, meta_title = ?, meta_description = ?, show_in_header = ?, sort_order = ? WHERE id = ?`, [
				c.name,
				finalSlug,
				c.description || "",
				c.metaTitle || "",
		...
		} else await query(`INSERT INTO categories (name, slug, description, meta_title, meta_description, show_in_header, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`, [
			c.name,
			finalSlug,
			c.description || "",
			c.metaTitle || "",
...
var getTags = createServerFn({ method: "GET" }).handler(getTags_createServerFn_handler, async () => {
	if (cachedTags && cachedTags.expiry > Date.now()) return cachedTags.data;
	try {
...
var saveTag = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((data) => data).handler(saveTag_createServerFn_handler, async ({ data }) => {
	clearTagsCache();
	const t = data;
...
var deleteTag = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((id) => id).handler(deleteTag_createServerFn_handler, async ({ data: id }) => {
	clearTagsCache();
...
var importTags = createServerFn({ method: "POST" }).middleware([requireAdmin]).validator((tags) => tags).handler(importTags_createServerFn_handler, async ({ data: tags }) => {
	clearTagsCache();
