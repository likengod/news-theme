...
var defaultFontConfig = {
	fonts: SYSTEM_FONTS,
	sectionMapping: {
		headlines: "sys-news-cycle",
		body: "sys-inter",
		navigation: "sys-inter",
		footer: "sys-inter",
		ticker: "sys-inter",
		buttons: "sys-inter"
	}
...
function buildGoogleFontsUrl(fonts) {
	...
	return `https://fonts.googleapis.com/css2?${googleFonts.map((font) => {
		...
		if (!font.weights || font.weights.length === 0) return `family=${name}`;
		return `family=${name}:wght@${[...font.weights].sort().join(";")}`;
	}).join("&")}&display=swap`;
}
...
function buildSectionCssVars(config) {
	...
	for (const section of FONT_SECTIONS) {
		...
		if (fontId) {
			...
			if (font) family = `"${font.family}", sans-serif`;
		}
